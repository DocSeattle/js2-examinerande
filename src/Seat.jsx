...
export default function Seat({i, occupied = false})
{
    ...
    return (
        <div id={i} className={`seat ${occupied ? "occupied" : ""}`} onClick={() => {
            ...
            if (target.classList.contains == "occupied") {return console.log("This seat is occupied")}
            ...
            */
            console.log(
                `seat #${i} has selected value: ${
                    target.classList.contains("selected") ? selected == 0 ? selected+1 : selected+0 : selected == 1 ? selected-1 : selected+0
                }`
                );
            console.log(select);
        }}
